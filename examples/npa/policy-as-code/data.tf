...
# Device classification tags (requires provider >= 0.4.2)
...
locals {
  ...
  # Device classification tag lookup (name -> ID map)
  device_tag_ids_by_name = {
    for t in data.netskope_device_classification_tag_list.all.tags :
    t.name => t.tag_id
  }

  # All available device classification tags
  device_classification_tags = data.netskope_device_classification_tag_list.all.tags
}
